...
wandb_project = 'owt'
wandb_run_name = 'gpt_yahoo' + '_run_' + str(time.time())
# data
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            X_EMBED, X_TOKENS, Y_TOKENS = get_batch_yahoo(batch_size, split=split, device=device)
            with ctx:
...
# training loop
X_EMBED, X_TOKENS, Y_TOKENS = get_batch_yahoo(batch_size, split="train", device=device) # fetch the very first batch
t0 = time.time()
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        X_EMBED, X_TOKENS, Y_TOKENS = get_batch_yahoo(batch_size, split="train", device=device)
